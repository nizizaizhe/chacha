...
export default class ChinaRegionWheelPicker extends BaseComponent {
  ...
  _filterCitys(province) {
    const provinceData = this._regionAllData.find(item => item.provinceName === province)
    return provinceData.cityList.map(item => item.cityName)
  }
  _filterAreas(province, city) {
    const provinceData = this._regionAllData.find(item => item.provinceName === province)
    const cityData = provinceData.cityList.find(item => item.cityName === city)
